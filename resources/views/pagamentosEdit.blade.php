...
<body>
    ...
    <div class="container pt-">
        ...
        <div class="border border-primary rounded px-3 py-3">
            <h4 class="mb-4">Editar Pagamento</h4>
            <form action="/pagamento/{{$pagamentos->id }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                <div class="form-group d-flex flex-row">
                    <label for="fornecedor" style="width: 80pt">Fornecedor :</label>
                    <input type="text" name="fornecedor" value="{{ $pagamentos->fornecedor }}" class="form-control input d-inline-flex">
                </div>

                <div class="form-group d-flex flex-row">
                    <label for="descricao" style="width: 80pt">Descrição :</label>
                    <input type="text" name="descricao" value="{{ $pagamentos->descricao }}" class="form-control">
                </div>

                <div class="form-group d-flex flex-row">
                    <label for="data" style="width: 80pt">Data :</label>
                    <input type="text" name="data" value="{{ $pagamentos->data }}" class="form-control">
                </div>

                <div class="form-group d-flex flex-row">
                    <label for="valor" style="width: 80pt">Valor :</label>
                    <input type="number" name="valor" value="{{ $pagamentos->valor }}" class="form-control">
                </div>
